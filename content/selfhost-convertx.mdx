...
`convertx.domain.tld`がNginxサーバーのIPアドレスを指すようにAレコードを設定します。

4. 動作確認

ブラウザで`https://convertx.domain.tld`にアクセスし、mTLS認証を突破すればConvertXのお目見えです。

変換可能な形式が大量に表示されますが、対象の形式を入力することで絞り込みが可能です。  
今回はwebpへ変換していますが、同じ形式への変換で複数のライブラリ(vips,ImageMagick,GraphicsMagick)が選択可能でした。

![11_convertx/01-convertx.webp](/images/11_convertx/01-convertx.webp)
![11_convertx/02-convertx.webp](/images/11_convertx/02-convertx.webp)
![11_convertx/03-convertx.webp](/images/11_convertx/03-convertx.webp)

変換自体も非常に高速で、あっという間に完了しました。

これで安心して、しかも簡単にファイル変換が可能になりました:)
