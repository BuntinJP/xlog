...
公式のObsidian Syncが料金を考えない場合は、最高の選択肢です。  
...
サードパーティー同期と呼ばれているプラグインを用いた同期で、最も良さそうなものをセルフホストする方向で進めます。  
サードパーティープラグインが多くあり、iPhoneなど縛り付けが厳しそうなプラットフォームでも利用できる所が、他のメモアプリに対してObsidianが大きくリードする点だと思います。

obsidian-headlessの存在は認知しています。  
正直がっかりしました。headlessというならば、プラグインまでシュミレートしろよと😭

## Self-hosted LiveSync

[obsidian-livesync](https://github.com/vrtmrz/obsidian-livesync)

Obsidianの、サードパーティープラグインになります。  
...
Syncthingとは、モバイル端末の内部ディレクトリを、rcloneのようにネットワーク上のストレージと同期するツールです。
...
マネージドホスティングは、私が探したところ(ChatGPT,Geminiに聞いたところ)、IBM Cloudantしかないとのことでした。  
無料枠1GBのその1つ上のプランが月額1万1000円のエンプラ向けアプライアンスのため、まあ正直眼中にありません。
...
コンテナをホストするならば、乞食的におすすめしたいのは「[zeabur](https://zeabur.com/)」です。こちらは今だに無料で毎月5ドル分のクオータがあります。
...
プラグインのドキュメントには存在しませんが、プラグイン設定からヘッダーによる認証を追加できそうだったので、そちらでアクセス制限を適用します。

また、Cloudflareは利用しません。また、推奨できません(地獄を見た)
...
4. 2台目端末用のセットアップURIを生成する

現在セットアップしている1台目の端末にて、2台目端末用のセットアップURIを生成できます。
...
パスフレーズURIが出てくるので、コピーして保存しておきます。
...
[Obsidian の同期を解消する & Cloudflare 配下でホストする](https://blog.naa0yama.com/p/18w10-azvs93f1/)
