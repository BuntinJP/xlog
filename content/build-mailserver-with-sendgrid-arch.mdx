...
description: さくらのメールボックスから移行します。ローカル隔離、バーチャルドメイン・ユーザー、一部別メールボックス構成、LMTPでかなりゴテゴテしたサーバーを構築します。設定のスパゲッティさでは世界一かもしれません。
date: 2024-12-14
...
しかし、システムメールなどは飛ばしたいため、ローカル配送はpostfixにやらせます。そのためpostfixもフルでセットアップします。ｓ
...
`main.cf` を変更し、LMTP設定をかけるように `transport_maps`に`/etc/postfix/transport`を指定します。
...
`/etc/postfix/transport`ですが、こいつは結構重要なファイルになります。  
dovecotのドキュメントの[postfix連携でのLMTP部分](https://doc.dovecot.org/2.3/configuration_manual/howto/postfix_dovecot_lmtp/)では、`virtual_transport`を利用するようになっています。  
この場合DovecotのLMTPに転送されるメールの条件は`virtual_mailbox_domains`などで定義する必要があります。
`main.cf`内でです。それだと設定がどうなっているのか分かりにくいです。特に今回のようにキモい設定を入れ込んでいる場合には特にです。  
そのため、全て一元的に定義できる`/etc/postfix/transport`を使用します。


```
root@badcompany /etc/postfix                                                                                                                               [1:53:34]
> # l | grep transport                                                                                                                                              
-rw-r--r--  1 root root  14K Dec  6 06:30 transport
root@badcompany /etc/postfix                                                                                                                               [1:53:46]
> # cp -p transport{,.$(date "+%Y%m%d")}                                                                                                                            
root@badcompany /etc/postfix                                                                                                                               [1:53:57]
> # echo "" > transport
root@badcompany /etc/postfix                                                                                                                               [1:53:57]
> # vim transport
root@badcompany /etc/postfix                                                                                                                               [2:02:26]
> # cat transport                                                                                                                                                   
badcompany.tokyo           lmtp:unix:private/dovecot-lmtp

mail.badcompany.tokyo      local:
```

ここで、`mail.badcompany.tokyo`は`$myhostname`、`$myorigin`です。
ローカル配送はこのドメインでやります。

フェーズ1のため単一ドメインのみ書きます。バーチャルドメイン設定などはややこしくなるのでフェーズ2で行います。

```
root@badcompany /etc/postfix                                 [2:09:21]
> # postmap lmdb:/etc/postfix/transport                               
root@badcompany /etc/postfix                                 [2:09:32]
> # ll | grep transport                                               
-rw-r--r-- 1 root root   93 Dec 17 02:02 transport
-rw-r--r-- 1 root root  14K Dec  6 06:30 transport.20241217
-rw-r--r-- 1 root root  12K Dec 17 02:09 transport.lmdb
```
...
root@badcompany /etc/postfix                                 [0:45:56]
> #   
```
