...
Syncthingは簡単に言うと、モバイル端末の内部ディレクトリをrcloneのようにネットワーク上のストレージと同期するツールです。
...
他のブログを見てると、`fly.io`とかいうホスティング会社がドキュメント中で触れられているためかそちらを利用してホストしている人が多めの印象です。  
しかし、注意して欲しいのは無料枠は最初の5ドル分のみというただの詐欺です。ご注意ください。

コンテナを乞食的にホストするならば、おすすめは「[zeabur](https://zeabur.com/)」です。こちらは今だに無料で毎月5ドル分のクオータがあります。
...
しかし、CouchDBに脆弱性が存在してゼロデイで突かれたら終わりな気がするので、気が向いたらTailscale経由に変更するかもです。

CouchDBのレスポンスを基準に、認証に失敗しまくっているIPをブロックしたり、ヘッダーを利用して二重に防御するとかしたいですが不可能です。
公式がホスティングを推奨していていて、ただのコンテナホスティングではWebサーバーの設定をあまり複雑にできないのが原因な気がします。
...
compose.ymlでユーザーへのパスワード設定を行っており、その情報(`CHAME_ME`)が外部からサーバーを守る唯一の手段(Basic認証😭)になります。
...
おそらくセットアップ中にセットアップURIが適切かどうかなどを判定するために、共通のパッケージとして利用していると思われます。
...
また、この手順では、パスワードを含めLiveSync関連設定をすべてVault内のmdファイルとして保存し、それごと同期するという方式前提の手順となってしまっています。  
パスワードをmdファイルとして保存したくない場合は、適宜読み替えてください。しかし、自分もその方針で試しましたが、同期に関して同様の設定を行ったとしても、なぜか色々トラブルが多かった印象です。
