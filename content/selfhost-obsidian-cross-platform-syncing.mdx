...
メモツールにこだわるのが結構好きなタイプなのですが、色々風呂敷を広げすぎて、自分で管理できなくなってしまいました笑

Apple Notes、Notion、Discord等、情報を記述するところが使用している端末によって情報が拡散しすぎたので、Obsidianにすべてを集約しようと思います。

かなり辛い選択ですが、どうにか適応します。

私にとって、一番必要なのはクロスプラットフォームであることです。  
現在の情報の拡散も、そこから来ています。

それを一番に重視すれば、Notionに確定してしまいますが、それだとLLMに読ませたりなど、そこからの拡張性にやはり限界があります。
...
じゃあObsidianもデフォルトで良いクロスプラットフォームなのかというと、そうでもありません。

公式のObsidian Syncが料金を考えない場合は、最高の選択肢です。  
しかし、公式へのお礼として当たり前に支払えるRedditの海外勢(そういうコメントを見かけるだけです)とは異なり、私は乞食です。  
なけなしの5ドル(年更新で4ドル)のサブスクは、ちょっと抵抗感があります。ランニングコストが、使い続けるというモチベに大きな抵抗となるので、サーバーをうまく利用してケチる方向で頑張ります。
...
Syncthingはファイルのハッシュ、mtimeを基準にするrsync方式ですが、Self-hosted LiveSyncの場合はObsidianの編集イベント駆動の同期が行われるようです。最高です。

内部でJSONデータベースとして、CouchDBを利用するようです(S3も選択可能だが実験的)。  
Apache CouchDBは、JSONドキュメントを保存するNoSQLデータベースであり、
すべての操作をHTTP REST API経由で行うデータベースサーバーです。

… Apache財団だからOKです笑

マネージドホスティングは、私が探したところ(ChatGPT,Geminiに聞いたところ)、IBM Cloudantしかないとのことでした。  
無料枠1GBのその1つ上のプランが月額1万1000円のエンプラ向けアプライアンスは正直眼中にありません。
...
- Self-hosted LiveSync用のCouchDB(10012ポート)へのリバースプロキシ設定
...
自分としてはDeno大好きなので最高ですが、念の為報告となります。
